 ...
 * */
 // $val1 = ".jpg";
     ...
     // if(strcasecmp($val1,$val2)==0)
        // echo '$val1和$val2相同(忽略字符串的大小写)';die;
refreshProductPic();
...
/**
    ...
    function getMainPicBySku($skuDir) {
		...
		if($fp) {
			$size=20000000002222222;
			while(false !== ($file = readdir($fp))) {
				
				//if($file!="." && $file!=".." && !is_dir($skuDir.$file) &&filesize($skuDir.$file)>20000&& substr($file, -5)=="1.jpg") {
				if($file!="." && $file!=".." && !is_dir($skuDir.$file) &&filesize($skuDir.$file)>20000&& strcasecmp(substr($file, -4),".jpg")==0) {
	...
	function getVicePicBySku($skuDir) {
		...
		if($fp) {
			...
			$viceFilePicNamemax[] = $filez;

			if ($viceFilePicName) {
				// var_dump($viceFilePicName);
				// var_dump($viceFilePicNamemax);
				$viceFilePicName=array_diff($viceFilePicName,$viceFilePicNamemax);
