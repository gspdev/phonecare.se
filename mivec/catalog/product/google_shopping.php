...
//<!-- AdWords attributes -->
	...
	$collection = '';				
		...
		if ($product->isSaleable()){
			...
			$this_value = '';
			$this_attrib = $product->getResource()->getAttribute($g_params_mapping['sku']);
			if(!empty($this_attrib)) $this_value = trim($this_attrib->getFrontend()->getValue($product));
			if($this_value !='')  {
				$collection .= 	"<g:sku>" . cdata($this_value) . "</g:sku>\n";  
			} 
			elseif ($force_mpn == 'on') {
				$collection .= 	"<g:sku>" . $product->getSku() . "</g:sku>\n";
			}
			
//id
			// $method = getMethod($g_params_mapping, 'sku');
			// if(!empty($method)){$value = $product->$method();} else {$value = '';}
		    // $collection .= 	"<g:id>" . cdata($value) . "</g:id>\n";
			
			// $method = getMethod($g_params_mapping, 'product_id');
			// if(!empty($method)){$value = $product->$method();} else {$value = '';}
		    // $collection .= 	"<g:product_id>" . $product->getId() . "</g:product_id>\n";
...
//			
			...
			// if(!empty($this_attrib)) $this_value = trim($this_attrib->getFrontend()->getValue($product));
			// //if($this_value !='')  {
			// //	$collection .= 	"<g:qty>" . cdata($this_value) . "</g:qty>\n";  
			// //} 
			// //elseif ($force_mpn == 'on') {
				// $collection .= 	"<g:qty>" . $product->getQty() . "</g:qty>\n";
			// //}
// //brand
			// $this_value = '';
