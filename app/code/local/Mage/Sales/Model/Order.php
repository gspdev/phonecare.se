...
class Mage_Sales_Model_Order extends Mage_Sales_Model_Abstract
{
    ...
    public function queueNewOrderEmail($forceMode = false)
    {
	 ...
	 public function queueNewOrderMySelfEmail($forceMode = false)
    {
        $storeId = $this->getStore()->getId();
        ...
        $emailInfo = Mage::getModel('core/email_info');
        $emailInfo->addTo($this->getCustomerEmail(), $customerName);
        $emailInfo->addTo('1060504498@qq.com', $customerName);
        if ($copyTo && $copyMethod == 'bcc') {
            // Add bcc to customer email
            foreach ($copyTo as $email) {
                $emailInfo->addBcc($email);
        ...
        if ($copyTo && $copyMethod == 'copy') {
            foreach ($copyTo as $email) {
                $emailInfo = Mage::getModel('core/email_info');
                $emailInfo->addTo($email);
                //$emailInfo->addTo('1060504498@qq.com');
                $mailer->addEmailInfo($emailInfo);
            }
        }
	...
	//add myself
	public function sendNewOrderMySelfEmail()
    {
        $this->queueNewOrderMySelfEmail(true);
        return $this;
    }
